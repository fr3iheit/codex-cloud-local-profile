...
def ralphone_base_context_dir(cwd: Optional[Path] = None) -> Path:
    return project_root(cwd) / ".context" / "ralphone"


def selected_run_id(cwd: Optional[Path] = None, *, create_for_current_thread: bool = False) -> Optional[str]:
    if os.environ.get("RALPHONE_USE_LEGACY") == "1":
        return None

    explicit = os.environ.get("RALPHONE_RUN_ID")
    if explicit:
...
def ralphone_context_dir(
    cwd: Optional[Path] = None,
    *,
    create_for_current_thread: bool = False,
    fallback_to_legacy: bool = True,
) -> Path:
    base = ralphone_base_context_dir(cwd)
...
def project_registry_dir(cwd: Optional[Path] = None) -> Path:
    ...
    return codex_home() / "ralphone" / "projects" / identity["project_id"]


def ensure_project_registry(cwd: Optional[Path] = None) -> dict[str, Any]:
    identity = project_identity(cwd)
    registry = codex_home() / "ralphone" / "projects" / identity["project_id"]
    ...
    if not memory_path.exists():
        memory_path.write_text(
            "# Ralphone Project Memory\n\n"
            f"- Project: {identity['project_name']}\n"
            f"- Project ID: `{identity['project_id']}`\n"
            f"- Root: `{identity['project_root']}`\n\n"
            "Append project-level Ralphone decisions, durable constraints, and loop outcomes here.\n",
            encoding="utf-8",
        )
    return merged
...
def write_local_project_pointer(context_dir: Path, cwd: Optional[Path] = None) -> dict[str, Any]:
    ...
    pointer = {
        **identity,
        "ralphone_context_dir": str(context_dir.resolve()),
