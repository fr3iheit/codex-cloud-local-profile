...
from ralphone_project import (
    ensure_project_registry,
    ralphone_base_context_dir,
    ralphone_context_dir,
    selected_run_id,
...
def context_dir() -> Path:
    return ralphone_context_dir(Path.cwd())
...
def progress_path() -> Path:
    return context_dir() / "progress.md"


def trace_path() -> Path:
    return context_dir() / "trace.jsonl"


def artifact_index_path() -> Path:
    return context_dir() / "artifact-index.json"


def logs_dir() -> Path:
    return context_dir() / "logs"
...
def append_trace(event_type: str, payload: dict[str, Any]) -> None:
    ensure_context_dirs()
    event = {
        "timestamp": now_iso(),
        "event": event_type,
...
def write_artifact_index(payload: dict[str, Any]) -> None:
    artifact_index_path().write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
...
def register_artifact(
    *,
    ...
    ensure_context_dirs()
    try:
        artifact = str(path.resolve().relative_to(context_dir()))
    except ValueError:
    ...
    append_trace("artifact_registered", entry)
...
def legacy_context_dir() -> Path:
    return ralphone_base_context_dir(Path.cwd())
...
def import_legacy_dag_for_unit(unit_id: str) -> Optional[dict[str, Any]]:
    ...
    add_note(
        imported,
        f"Imported legacy project-level Ralphone DAG into run {imported.get('run_id')} because unit {unit_id} was requested explicitly.",
    )
    save_dag(imported)
    return imported
            ...
            json.dumps(
                {
                    ...
                    "forbidden_paths": [],
                    "non_goals": [],
                    "notes": "Populate with ralphone_state.py set-scope before running scope-check.",
                },
                indent=2,
            )
            + "\n",
...
def sync_review_input_artifacts(unit: dict[str, Any]) -> bool:
    ...
    viability_artifacts = []
    scope_artifacts = []
    for check_name in ("baseline_recorded", "viability_gate_passed", "scope_guard_passed", "targeted_checks_passed"):
...
def normalize_unit(dag: dict[str, Any], unit: dict[str, Any]) -> None:
    ...
    scope.setdefault("allowed_paths", [])
    scope.setdefault("forbidden_paths", [])
...
def unit_scope_path(unit_id: str) -> Path:
    return unit_dir(unit_id) / "scope.json"
...
def normalize_scope_entry(value: str) -> str:
    ...
    text = text.replace(os.sep, "/").strip("/")
    return text or "."
...
def load_unit_scope(unit: dict[str, Any]) -> dict[str, Any]:
    scope = {
        "allowed_paths": [],
        "forbidden_paths": [],
        "non_goals": [],
    }
    path = unit_scope_path(unit["id"])
    if path.exists():
        try:
            ...
            pass
    scope.update(unit.get("scope") or {})
    scope["allowed_paths"] = [normalize_scope_entry(p) for p in scope.get("allowed_paths", []) if normalize_scope_entry(p)]
    scope["forbidden_paths"] = [normalize_scope_entry(p) for p in scope.get("forbidden_paths", []) if normalize_scope_entry(p)]
    scope["non_goals"] = [str(p) for p in scope.get("non_goals", [])]
    return scope


def write_unit_scope(unit: dict[str, Any], scope: dict[str, Any]) -> None:
    unit["scope"] = {
    ...
    path = unit_scope_path(unit["id"])
    path.write_text(json.dumps(unit["scope"], indent=2) + "\n", encoding="utf-8")


def git_changed_files() -> tuple[list[str], Optional[str]]:
    root = subprocess.run(
    ...
    for line in result.stdout.splitlines():
        ...
        if path.startswith(".context/ralphone/"):
            continue
        changed.append(path)
    return sorted(set(changed)), None


def evaluate_scope_guard(unit: dict[str, Any]) -> dict[str, Any]:
    scope = load_unit_scope(unit)
    changed, error = git_changed_files()
    violations: list[dict[str, str]] = []
    if error:
        violations.append({"path": "<workspace>", "reason": error})
    if not scope["allowed_paths"]:
        violations.append({"path": "<scope>", "reason": "allowed_paths is empty; run set-scope first"})
    for path in changed:
        if scope["allowed_paths"] and not any(path_under(path, allowed) for allowed in scope["allowed_paths"]):
            violations.append({"path": path, "reason": "outside allowed_paths"})
        if any(path_under(path, forbidden) for forbidden in scope["forbidden_paths"]):
            violations.append({"path": path, "reason": "inside forbidden_paths"})
    return {
        "scope": scope,
...
def compact_progress_text(dag: dict[str, Any], archive_path: Optional[Path]) -> str:
    active = active_units(dag)
    lines = [
        "# Ralphone Progress",
        "",
    ...
    if archive_path is not None:
        lines.append(f"- Previous progress archived at: `{archive_path}`")
    lines.extend(["", "## Active Units"])
    if active:
        for unit_id in active:
            unit = dag["units"][unit_id]
            lines.append(f"- `{unit_id}`: {unit.get('title', '')} ({unit.get('tier', 'unknown')})")
    else:
        lines.append("- none")

    blocked = [unit_id for unit_id, unit in dag.get("units", {}).items() if unit.get("status") == "blocked"]
    lines.extend(["", "## Blocked Units"])
    if blocked:
        for unit_id in sorted(blocked):
    ...
    for unit_id in sorted(dag.get("units", {})):
        ...
        for check_name in CHECK_NAMES:
            ...
            value = check.get("value", "fail")
            summary = check.get("summary", "")
            marker = "OK" if check_satisfied(check_name, value) else "RED"
            lines.append(f"- {marker} `{check_name}` = `{value}` {summary}".rstrip())
        review = ensure_review_state(unit)
        lines.append(f"- review_state = `{review.get('status')}` verdict=`{review.get('verdict')}`")

    index = load_artifact_index()
    recent_artifacts = index.get("artifacts", [])[-12:]
    lines.extend(["", "## Recent Artifacts"])
    if recent_artifacts:
        for item in recent_artifacts:
            lines.append(
                f"- {item.get('timestamp')} `{item.get('kind')}`"
                f" unit=`{item.get('unit_id')}` check=`{item.get('check_name')}` path=`{item.get('path')}`"
            )
    else:
        lines.append("- none")

    note_lines: list[str] = []
    for note in dag.get("notes", [])[-5:]:
        note_lines.append(f"- {note.get('timestamp')}: {note.get('message')}")
    for unit_id in sorted(dag.get("units", {})):
        for note in dag["units"][unit_id].get("notes", [])[-3:]:
            note_lines.append(f"- {note.get('timestamp')} `{unit_id}`: {note.get('message')}")
    lines.extend(["", "## Recent Notes"])
    lines.extend(note_lines[-12:] or ["- none"])

    lines.extend(["", "## Next Commands"])
    lines.append(f"- status: `{state_script_command()} status`")
    if active:
        unit_id = active[0]
        lines.append(f"- next check: `{state_script_command()} run-check {unit_id} targeted_checks_passed --command \"<command>\" --expect-exit 0`")
    return "\n".join(lines) + "\n"


def validate_compact_progress(dag: dict[str, Any], text: str) -> list[str]:
    missing: list[str] = []
    for unit_id in active_units(dag):
        if unit_id not in text:
            missing.append(f"active unit missing: {unit_id}")
    for unit_id, unit in dag.get("units", {}).items():
        if unit.get("status") == "blocked" and unit_id not in text:
            missing.append(f"blocked unit missing: {unit_id}")
        for check_name in CHECK_NAMES:
            check = unit.get("checks", {}).get(check_name, {})
            value = check.get("value", "fail")
            if not check_satisfied(check_name, value) and (unit_id not in text or check_name not in text):
                missing.append(f"red gate missing: {unit_id}/{check_name}")
    return missing


def cmd_bootstrap(_: argparse.Namespace) -> int:
    dag = load_dag()
    add_note(dag, "ralphone_state.py bootstrap called.")
    save_dag(dag)
...
def cmd_set_scope(args: argparse.Namespace) -> int:
    ...
    unit = get_unit(dag, args.unit_id)
    scope = {
        "allowed_paths": [normalize_scope_entry(p) for p in (args.allowed or []) if normalize_scope_entry(p)],
        "forbidden_paths": [normalize_scope_entry(p) for p in (args.forbidden or []) if normalize_scope_entry(p)],
        "non_goals": args.non_goal or [],
    }
    write_unit_scope(unit, scope)
    register_artifact(
        unit_id=unit["id"],
        kind="scope",
        path=unit_scope_path(unit["id"]),
        summary="Scope guard configured",
    )
    add_note(unit, "Scope guard configured.")
    refresh_allow_stop(dag)
    save_dag(dag)
    print(f"Scope configured for {unit['id']}: {unit_scope_path(unit['id'])}")
    return 0
...
def cmd_scope_check(args: argparse.Namespace) -> int:
    ...
    dag = load_dag()
    unit = get_unit(dag, args.unit_id)
    evaluation = evaluate_scope_guard(unit)
    artifact_path = artifacts_dir / f"scope_guard_passed-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ')}.json"
    artifact = {
        "unit_id": args.unit_id,
        "check_name": "scope_guard_passed",
        "command": f"{state_script_command()} scope-check {args.unit_id}",
        "started_at": now_iso(),
        **evaluation,
    }
    artifact["finished_at"] = now_iso()
    artifact_path.write_text(json.dumps(artifact, indent=2) + "\n", encoding="utf-8")
    value = "pass" if evaluation["passed"] else "fail"
    summary = "Scope guard passed." if evaluation["passed"] else f"Scope guard violations: {len(evaluation['violations'])}"
    update_check(
        unit,
        "scope_guard_passed",
        value,
    ...
    unit.setdefault("artifacts", {}).setdefault("scope_guard_passed", []).append(str(artifact_path))
    invalidate_review_if_stale(unit)
    sync_review_input_artifacts(unit)
    add_note(unit, f"scope-check -> {value}.")
    refresh_allow_stop(dag)
    save_dag(dag)
    print(f"{args.unit_id} scope_guard_passed = {value}")
    print(f"artifact: {artifact_path}")
...
def cmd_compact_progress(_: argparse.Namespace) -> int:
    dag = load_dag()
    archive_path: Optional[Path] = None
    if progress_path().exists():
        archive_path = logs_dir() / f"progress-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ')}.md"
        shutil.copy2(progress_path(), archive_path)
    text = compact_progress_text(dag, archive_path)
    missing = validate_compact_progress(dag, text)
    if missing:
        raise SystemExit("Refusing to compact progress; summary would lose: " + "; ".join(missing))
    progress_path().write_text(text, encoding="utf-8")
    if archive_path is not None:
        register_artifact(unit_id=None, kind="progress-archive", path=archive_path, summary="Pre-compaction progress archive")
    register_artifact(unit_id=None, kind="progress-current", path=progress_path(), summary="Compacted progress dashboard")
    add_note(dag, "Compacted progress.md from canonical state.")
    save_dag(dag)
    print(f"Compacted progress: {progress_path()}")
    if archive_path is not None:
...
def cmd_block(args: argparse.Namespace) -> int:
    ...
    quality_incomplete = any(
        checks.get(name, {}).get("value") != "pass"
        for name in ("scope_guard_passed", "targeted_checks_passed", "review_complete")
    )
    if (
...
def cmd_finish_loop(_: argparse.Namespace) -> int:
    ...
    dag["active_units"] = []
    refresh_allow_stop(dag)
    add_note(dag, "Finished Ralphone loop.")
    save_dag(dag)
    print("Ralphone loop finished.")
    return 0


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Manage Ralphone loop state.")
    parser.add_argument("--run-id", help="Use .context/ralphone/runs/<run-id> instead of the thread/default run.")
    parser.add_argument("--legacy", action="store_true", help="Use the legacy project-level .context/ralphone state.")
    ...
    set_scope = subparsers.add_parser("set-scope")
    set_scope.add_argument("unit_id")
    set_scope.add_argument("--allowed", nargs="*", default=[])
    set_scope.add_argument("--forbidden", nargs="*", default=[])
    set_scope.add_argument("--non-goal", action="append", default=[])

    scope_check = subparsers.add_parser("scope-check")
    scope_check.add_argument("unit_id")
    ...
    subparsers.add_parser("compact-progress")
...
def main() -> int:
    ...
    if args.run_id:
        os.environ["RALPHONE_RUN_ID"] = args.run_id
    if args.legacy:
        os.environ["RALPHONE_USE_LEGACY"] = "1"
    ...
    handlers = {
        ...
        "ready": cmd_ready,
        "compact-progress": cmd_compact_progress,
