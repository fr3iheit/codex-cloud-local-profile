---
name: ralphone
description: Run a repo-agnostic Ralphone loop for complex builds, fixes, or refactors in Codex. Use when the user wants autonomous execution, explicit quality gates, and a harness-first loop with artifact-backed state.
metadata:
...
Every Ralphone run must use isolated run state. In Codex Desktop, new chats normally select `.context/ralphone/runs/<CODEX_THREAD_ID>/...`; manual parallel runs can set `RALPHONE_RUN_ID=<name>`. Use `RALPHONE_USE_LEGACY=1` only when intentionally resuming a pre-existing legacy `.context/ralphone/dag.json`.
...
Ralphone state is project- and run-scoped. A Ralphone loop is active for the current chat only when it is active in the selected Ralphone run, meaning one of these is true:

- the current chat's project root or working directory contains the relevant `.context/ralphone/runs/<run-id>/dag.json`
- the user explicitly asks to resume a named Ralphone loop, handoff, unit, or workspace
- this same thread already launched or accepted the Ralphone unit being continued

Do not search other runs, other projects, global memory, session history, or unrelated worktrees for active Ralphone DAGs when deciding what a new chat should do. An active Ralphone loop in a different run or project is context, not a command. Mention it only when the user asks about it or it is directly relevant, then continue with the user's current request.

This isolation rule must not break running Ralphone loops: do not stop processes, delete state, rewrite another run's `.context/ralphone`, or weaken evidence gates just because a new unrelated chat started.

## Continuous auto-resume rule

When Ralphone is active in the current selected Ralphone run, it must keep going until every mandatory gate for the active unit or dependency chain is green, or the user explicitly says to stop.

Treat these as operational interruptions, not as valid stop conditions:

- the assistant being redirected to a related side question inside the same Ralphone run while the loop is still open
- a stop hook reminder
- a tool timeout or bounded wait
- a session boundary or new chat that is explicitly continuing the same Ralphone run
- a partial green state such as implementation passing while review is still red

Required behavior while a loop is active:

- update Ralphone state when a new blocker, regression, or real-user failure is discovered
...
Inside the current selected Ralphone run, do not treat "Ralphone is still active in the DAG" as merely informative state. It is a command to continue that loop. Outside the current run, follow the project/worktree isolation rule above.
...
The first job is to infer the user's actual success function, not the easiest
thing to validate. Ask: if the user saw the finished artifact cold, what would
they say had gone well or badly? Encode that answer in the contract before
implementation.
...
- the **user success function** in plain language: what the user, reader, or downstream
  operator should understand, decide, or do after consuming the artifact
- the **forbidden proxy passes**: things that may look polished but do not actually satisfy
  the user's goal
...
### Explainer / knowledge-artifact rule

When the artifact is meant to explain a repo, product, workflow, system, model,
study, or business to a human reader, the DoD must be reader-centered first and
artifact-centered second.

The contract must state:

- who the target reader is
- what a zero-context reader should understand after a short first pass
- what a deeper reader should be able to learn after a longer pass
- what would count as confusion, overload, or misleading simplification
- what public-facing content must stay out unless the user explicitly wants it
  inside the artifact, such as deploy instructions, implementation chatter,
  meta-commentary about the site/document itself, or generic marketing filler

For these artifacts, require both of the following oracles unless the user
explicitly waives one:

- **blind-reader oracle**: reviewer sees only the rendered/public artifact, no
  source code or repo context, and reports:
  - what the artifact is
  - what a newcomer would think is live today
  - how it supposedly works or makes money
  - what remains unclear
  - a clarity score
- **code-aware adjudicator oracle**: reviewer sees the artifact plus the source
  repo/docs and reports:
  - whether the blind reader's understanding is accurate
  - what important truths were omitted, distorted, or over-emphasized
  - whether the artifact is optimizing real comprehension or merely tidy structure

For these artifacts, examples of invalid completion evidence include:

- the page has the right sections
- the site renders and screenshots well
- data/file paths are present
- a validator confirms files, IDs, or links
- the artifact is deployable

Those are smoke checks only. They do not prove that a smart newcomer now
...
Ralphone must not complete a unit by proving only that files exist, schemas parse, counts
...
- an "AI playtest" that is only a heuristic script when the user requested model
  players
- an explainer site that has structure, examples, and visuals but still leaves a
  zero-context reader unsure what the product is, for whom it exists, how it
  makes money, or what is actually live today

When the requested artifact is a game or interactive system, Ralphone must require:
...
When Ralphone is used to make an LLM product or agentic chat production-ready, the
...
2. Inspect the selected repo-local Ralphone run:
   - `.context/ralphone/plan.md`
   - `.context/ralphone/model-routing.md`
   - `.context/ralphone/review-rubric.md`
   - `.context/ralphone/runs/<run-id>/dag.json`
...
```bash
python3 "<skill-root>/scripts/ralphone_state.py" add-unit <unit-id> --title "<title>" --tier <tier> --deps dep-a dep-b
```

## Runtime layout

Ralphone now treats `.context/ralphone/runs/<run-id>` as the harness root for new
runs. The project root is resolved with `git rev-parse --show-toplevel` when
possible, so running Ralphone from a subdirectory such as `web/` still uses the
same project identity, but each Codex thread or explicit `RALPHONE_RUN_ID` gets its
own run state.

- `.context/ralphone/runs/<run-id>/dag.json`: run loop state, unit map, active units
- `.context/ralphone/runs/<run-id>/project.json`: pointer to the project-scoped global Ralphone memory
- `.context/ralphone/runs/<run-id>/units/<unit-id>/state.json`: per-unit state
- `.context/ralphone/runs/<run-id>/units/<unit-id>/contract.md`: unit contract
- `.context/ralphone/runs/<run-id>/units/<unit-id>/planner-output.json`: planner artifact
- `.context/ralphone/runs/<run-id>/units/<unit-id>/executor-output.json`: implementation handoff
- `.context/ralphone/runs/<run-id>/units/<unit-id>/review-input.json`: narrow reviewer payload
- `.context/ralphone/runs/<run-id>/units/<unit-id>/artifacts/*.json`: command-backed evidence

Legacy `.context/ralphone/dag.json` remains readable only for old runs. Use
`RALPHONE_USE_LEGACY=1` when intentionally resuming that state.

Global Ralphone memory is project-scoped under:

- `$CODEX_HOME/ralphone/projects/<project_id>/project.json`
- `$CODEX_HOME/ralphone/projects/<project_id>/memory.md`
- `$CODEX_HOME/ralphone/projects/<project_id>/logs/`

Do not store cross-project Ralphone decisions in a single shared memory file. If a
task spans multiple repositories, record the project-local evidence in each
project's Ralphone context and use a separate coordination unit for cross-project
state.
...
```bash
python3 "<skill-root>/scripts/ralphone_state.py" check contract_written pass --unit <unit-id> --artifact-path .context/ralphone/runs/<run-id>/units/<unit-id>/contract.md --summary "Contract written"
```
...
```bash
python3 "<skill-root>/scripts/ralphone_state.py" run-check <unit-id> baseline_recorded --command "<oracle command expected to fail>" --expect-exit <nonzero>
```
...
```bash
python3 "<skill-root>/scripts/ralphone_state.py" run-check <unit-id> viability_gate_passed --command "<coverage/provenance/query command>" --expect-exit 0
```

If viability is not relevant to the unit, mark it explicitly:

```bash
python3 "<skill-root>/scripts/ralphone_state.py" check viability_gate_passed skip --unit <unit-id> --summary "No external dataset/filter viability risk in this unit."
```
...
```bash
python3 "<skill-root>/scripts/ralphone_state.py" run-check <unit-id> targeted_checks_passed --command "<narrow passing command>" --expect-exit 0
```

8. Use `check ... pass` only when the evidence is a file rather than a command:

```bash
python3 "<skill-root>/scripts/ralphone_state.py" check review_complete pass --unit <unit-id> --artifact-path .context/ralphone/runs/<run-id>/units/<unit-id>/review-verdict.md --summary "Review artifact completed"
```

9. When required checks are green, mark the unit complete:

```bash
python3 "<skill-root>/scripts/ralphone_state.py" complete-unit --unit <unit-id>
```

10. When landed:

```bash
python3 "<skill-root>/scripts/ralphone_state.py" land-unit <unit-id> --commit <sha>
```

11. Inspect runnable work:

```bash
python3 "<skill-root>/scripts/ralphone_state.py" ready
python3 "<skill-root>/scripts/ralphone_state.py" status
```

If blocked:

```bash
python3 "<skill-root>/scripts/ralphone_state.py" block "reason" --unit <unit-id> --terminal
```
...
Ralphone is harness-first. A green check should point to evidence:
...
For any unit that trains, evaluates, migrates, or depends on filtered inputs, Ralphone must prove the thing still exists before spending time on the next expensive step.
...
Ralphone should not fan out by default.
...
- when Ralphone is the requested workflow, launch it with a prompt that explicitly says to
  continue the active unit in the current selected Ralphone run until all mandatory gates pass or the user explicitly stops it
